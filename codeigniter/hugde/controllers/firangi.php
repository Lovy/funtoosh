...
class firangi extends CI_Controller{
	...
	function index(){
		...
		if(!empty($LoginFlag)){
			...
			$response['huggas'] = $this->modelhome->loadData('0',$data['userId'],'HIDE',4,1,'Firangi');   //(huggasPerPage,pageNo)
			$response['sidebar'] = $this->modelhome->loadSideBar();
			$response['data']=$data;
			$response['category']='Firangi';
			...
			$this->load->model('modelhome');
			$response['huggas'] = $this->modelhome->loadData('0',NULL,'HIDE',4,1,'Firangi');
			$response['sidebar'] = $this->modelhome->loadSideBar();
