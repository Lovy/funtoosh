...
class desi extends CI_Controller{
	...
	function index(){
		...
		if(!empty($LoginFlag)){
			...
			$response['huggas'] = $this->modelhome->loadData('0',$data['userId'],'HIDE',4,1,'Desi');   //(huggasPerPage,pageNo)
			$response['sidebar'] = $this->modelhome->loadSideBar();
			$response['data']=$data;
			$response['category']='Desi';
			...
			$this->load->model('modelhome');
			$response['huggas'] = $this->modelhome->loadData('0',NULL,'HIDE',4,1,'Desi');
			$response['sidebar'] = $this->modelhome->loadSideBar();
